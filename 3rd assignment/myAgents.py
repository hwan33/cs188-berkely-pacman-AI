...
class MyAgent(Agent):
    ...
    def getAction(self, state):
        ...
        idx = self.index
        act = self.action
        CG = self.currentGoals

        if idx not in act.keys():
            act[idx] = BFS(AnyFoodSearchProblem(state, idx), CG[0])
            if act[idx][0] == ['Stop']:
                return 'Stop'
            CG[0].add(act[idx][1])

        if len(act[idx][0]) == 0:
            CG[0].discard(act[idx][1])
            act[idx] = BFS(AnyFoodSearchProblem(state, idx), CG[0])
            if act[idx][0] == ['Stop']:
                return 'Stop'
            CG[0].add(act[idx][1])
